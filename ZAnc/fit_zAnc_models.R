...
FDRs <- bind_rows(calc_FDR(d = fits$diff_elev_ll2, s = fits_sim$diff_elev_ll2, 
                 FDR_values = FDR_range, 
                             test_values = seq(min(fits$diff_elev_ll2), 
                                               max(fits$diff_elev_ll2), 
                                               by = .0001)) %>%
                  dplyr::mutate(model = "elev"),
                calc_FDR(d = fits$diff_all_ll2, s = fits_sim$diff_all_ll2, 
                         FDR_values = FDR_range, 
                         test_values = seq(min(fits$diff_all_ll2), 
                                           max(fits$diff_all_ll2), 
                                           by = .0001)) %>%
                  dplyr::mutate(model = "all"),
                calc_FDR(d = fits$zTz, s = fits_sim$zTz, 
                         FDR_values = FDR_range, 
                         test_values = seq(min(fits$zTz), 
                                           max(fits$zTz), 
                                           by = .0001)) %>%
                  dplyr::mutate(model = "zTz")) %>%
  dplyr::filter(., tail == "high")
