...
# X is a vector of ones representing a shared selection environment across all populations [1,1,1,1....1]^T
# returns b estimate for each snp
...
# environment
X = rep(1, length(alpha)) # vector of ones
X = as.matrix(X)
...
write.table(fits,
            paste0("results/models/", PREFIX, "/maize/ML_allpop.txt"),
            sep = "\t",
            quote = F,
            col.names = T, 
            row.names = F)
