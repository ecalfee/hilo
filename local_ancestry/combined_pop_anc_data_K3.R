...
output_parv_anc = snakemake@output[["parv_anc"]]
...
# parviglumis ancestry
anc_parv = do.call(cbind,
                lapply(meta_pops$popN,
                       function(i) read.table(paste0(dir_anc, "/parv/pop", i, ".anc.freq"),
                                          header = F, stringsAsFactors = F)$V1))
colnames(anc_parv) = meta_pops$pop
...
meta_pops$alpha_local_ancestry_parv = apply(anc_parv, 2, mean)
...
anc_parv_mean = (anc_parv %*% meta_pops$n_local_ancestry)/sum(meta_pops$n_local_ancestry)

# write output files:

# RData files
save(list = "meta_pops", file = output_meta_pop)
save(list = c("anc_mexicana", "anc_maize", "anc_parv", "anc_mexicana_mean", "anc_maize_mean", "anc_parv_mean"), file = output_pop_anc)
...
write.table(tracts %>%
                ...
                col.names = T, row.names = F, quote = F, sep = "\t")
write.table(tracts %>%
                mutate(., anc_freq = anc_parv_mean),
                file = output_parv_anc,
                col.names = T, row.names = F, quote = F, sep = "\t")
