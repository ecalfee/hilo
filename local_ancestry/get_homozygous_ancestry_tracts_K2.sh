...
# to run: get_homozygous_ancestry_tracts_K2.sh {input.bed} {params.ZEA} {input.ploidy} {params.dir_post} {params.dir_out}
...
for (( i=0; i<$n_ids; i++ )); do
    ...
    if [ "$ZEA" == "maize" ]; then # find homozygous ancestry state for maize
        echo "finding maize tracts $ID"
		    # posterior probability for homozygous maize ancestry is the 3rd column of ancestry_hmm posterior output file
        ...
        # posterior probability for homozygous mexicana ancestry is the 5th column of ancestry_hmm posterior output file
