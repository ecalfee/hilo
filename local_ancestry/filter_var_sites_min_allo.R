...
# args (in order): region, minInd_maize, minInd_maize, D, allo_maize, allo_mex, dir_maf, dir_sites, dir_out
# to run:
...
minInd_maize = as.integer(args[2])
minInd_mex = as.integer(args[3])
#D = 0.3
D = as.numeric(args[4])
...
getSNPs = function(region, dir_maf, dir_sites, allo_maize, allo_mex, D, minInd_maize, minInd_mex){
  ...
  d = read.table(paste0(dir_sites, "/", "region_", region, ".var.sites"),
    ...
    filter(., nInd_maize >= minInd_maize & nInd_mex >= minInd_mex) %>% # thin SNPs based on min # ind's in allopatric pops
...
d = getSNPs(region = region, dir_maf = dir_maf,
            ...
            allo_mex = allo_mex, D = D, minInd_maize = minInd_maize,
            minInd_mex = minInd_mex)
