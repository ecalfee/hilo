...
#SBATCH -J getHom
#SBATCH -o /home/ecalfee/hilo/slurm-log/getHomozygTractsBams_%A_%a.out
#SBATCH -t 2-00:00:00
#SBATCH --mem=16G
#SBATCH --array=1-200
# to run: sbatch --export=DIR_BAM=../filtered_bams/results/pass1,DIR_POST=../data/geno_lik/merged_pass1_all_alloMaize4Low_16/thinnedHMM/ancestry_hmm/output_noBoot get_homozyg_tracts_and_bams.sh
...
# and merges tracts together with high probability homozygosity for maize or mexicana ancestry
...
mkdir -p "results/input/maize2/tracts"
mkdir -p "results/input/maize2/bams"
...
if [ ! -f "$DIR_POST"/HILO"$i".posterior ]; then
  ...
  # getting high posterior probability tracts homozygous for maize ancestry
  ...
  # filtering bam for homozygous maize regions
  echo "filtering bam for homozygous maize regions"
  bedtools intersect -sorted -a "$DIR_BAMS"/HILO"$i".sort.dedup.baq.bam \
  -b results/input/maize2/tracts/HILO"$i".bed > results/input/maize2/bams/HILO"$i".sort.dedup.baq.bam
